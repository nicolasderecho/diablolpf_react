...
import RawCubeRecipes from '../data/json/cube_recipes.json';
import DiabloTable from '../shared/DiabloTable';
import { isBlank, matchesArrayFilter } from '../shared/helpers/util';
import type { CubeFormula } from '../types';

const TABLE_HEADERS = ['Formula', 'Resultado', 'Detalles'];
const CubeRecipes = RawCubeRecipes as CubeFormula[];

const CubeFormulas = () => {
  const [cubeFormulas, setCubeFormulas] = useState<CubeFormula[]>(CubeRecipes);
  const [item, setItems] = useState('');
  const [result, setResults] = useState('');
  const [description, setDescription] = useState('');

  const renderRow = (cubeFormula: CubeFormula, index: number) => (
    <TableRow key={index}>
  ...
  const matchesItem = (formula: CubeFormula): boolean =>
    matchesArrayFilter(item, formula.items);
  const matchesResult = (formula: CubeFormula): boolean =>
    isBlank(result) || !!formula.result.toLowerCase().match(result.toLowerCase());
  const matchesDescription = (formula: CubeFormula): boolean =>
    isBlank(description) || !!formula.description.toLowerCase().match(description.toLowerCase());
  const matchesFilters = (formula: CubeFormula): boolean =>
    matchesItem(formula) &&
    matchesResult(formula) &&
    matchesDescription(formula);
  const filterFormulas = () =>
    setCubeFormulas(CubeRecipes.filter((formula) => matchesFilters(formula)));
