...
function tcColor(tcNumber) {
  if (tcNumber >= 60 && tcNumber <= 70) return 'text-diablo-blue font-bold';
  if (tcNumber > 70 && tcNumber < 85) return 'text-diablo-orange font-bold';
  if (tcNumber >= 85) return 'text-diablo-yellow font-bold';
  return '';
}
...
const ActAreas = ({ actData, title, className = '', startOpened = false }) => {
  ...
  return (
    <div className={classes}>
      ...
      <div
        className={cn('collapsible-content overflow-hidden transition-[max-height] duration-150 ease-out', isOpened ? 'max-h-[2500px]' : 'max-h-0')}
      >
          ...
          <tbody>
            {actData.map((row) => (
              <TableRow key={row.name + row.originalName}>
                ...
                <td className={cn('diablo-table-item table-cell text-center', tcColor(row.normalTc))}>{row.normalTc}</td>
                <td className={cn('diablo-table-item table-cell text-center', tcColor(row.nightmareTc))}>{row.nightmareTc}</td>
                <td className={cn('diablo-table-item table-cell text-center', tcColor(row.hellTc))}>{row.hellTc}</td>
              </TableRow>
            ))}
          </tbody>
        </table>
